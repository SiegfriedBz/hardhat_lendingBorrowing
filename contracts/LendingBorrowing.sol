...
contract LendingBorrowing {
  ...
  constructor(uint256 _interestRate) {
    if (_interestRate == 0 || _interestRate >= 10**18) { // interestRate must be ]0,1[
  ...
  function borrrow(uint256 _amount) public payable {
    ...
    for(uint256 i=0; i<lenders.length; i++) {
      (uint256 debtToLender, uint256 debtWithInterestToLender) =
      calculateLenderData(lenders[i], _amount, totalDebt);
      lenderToBalance[lenders[i]] -= debtToLender;
      borrowerLoans[borrowerLoansCounter].lenderToDebt[lenders[i]] = debtWithInterestToLender;
    }
  ...
  function calculateLenderData(address _lender,
      uint256 _amount,
      uint256 _totalDebt) internal view returns(uint256, uint256) {
    uint256 ratio = lenderToBalance[_lender] * 10**2 / address(this).balance;
  ...
  function getBorrowerActiveLoansNumber() public view returns(uint256) {
